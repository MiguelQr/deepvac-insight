...
reports, simulator, monitor, opc) are all built once at DeepVacDesktop
construction time and live in a QStackedWidget -- _nav_to() only changes
the visible index, it never reconstructs a page. So the realistic risk
this check guards against (reconnecting the same signal on every visit)
doesn't actually exist in this navigation model; the test below confirms
that's genuinely true (receiver counts stay constant) rather than
asserting something that can't fail as written.
"""
...
def test_opc_page_navigable(window):
    window._nav_to(6)
    assert window.content_stack.currentIndex() == 6
